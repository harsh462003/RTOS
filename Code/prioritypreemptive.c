...
int main() {
    int n, i, completed_count = 0, current_time = 0, prev = -1;
    ...
    while (completed_count < n) {
        ...
        // Find highest priority process that has arrived and not finished
        for (i = 0; i < n; i++) {
        ...
        if (idx == -1) {
            // CPU idle
            if (prev != 0) {
                gantt_processes[gantt_index++] = 0;  // 0 for idle
